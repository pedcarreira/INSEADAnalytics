...
**Your Answers here:**
<br>
1. 6 factors explain have a Cumulative Proportion Explained of 52%
<br>
2. The highest weight is 0.93
<br>
3. Interpretation is that the bonds, currencies, stock indices/aggregators (#3 and 4), and interest rates are the principal determinants of tradable securities. These could be called the principal risk factors of the economy.
<br>
4.<br>
For the pre_crisis period (2002-11-01 to 2007-10-01) 50% of the variation is explained by 6 factors and the 1st component explains only 17% of it: <br>
...
Rotated_Results<-principal(pre_crisis_data, nfactors=20, rotate="varimax",score=TRUE)
Rotated_Factors<-round(Rotated_Results$loadings,2)
Rotated_Factors<-as.data.frame(unclass(Rotated_Factors))
colnames(Rotated_Factors)<-paste("Component",1:ncol(Rotated_Factors),sep=" ")
...
<br>
For the post_crisis period (2002-11-01 to 2007-10-01) 50% of the variation is concentraded in only 4 factors and the 1st component (mainly government bonds) explains already 30% of the variation:<br>
```{r echo=FALSE, comment=NA, warning=FALSE, error=FALSE,message=FALSE,results='asis'}
...
<br>
5. In bear markets/periods the overall market performance is much more impacted by the regulator's mechanisms to shape the market (low-risk government bond rates are the 1st factor, regulated borrowing rates such as Euribor are the 2nd and 3rd factors), than in bull markets/periods."
<hr>
...
**Your Answers here:**
<br>
1. The code basically looks to the moving average of the past 20 days (assigning +1 if it is long and -1 if it is short in each of the days) and uses this average result to predict what is the best strategy to follow
<br>
2. We would get different growth trends as one changes the moving average window (increasing or reducing from 20). For 10 and 30 days the returns are slightly better (Ret:6.3, for aprox. the same Volatility and Sharpe ratio as before). However for 60 days or even larger windows of time the returns start to decline (Ret:5.9 for 60 days). 
<br>
<br>
